...
def forward_backward(y, X, betas, sigmas, P, pi):
    ...
    return gamma, xi, log_likelihood # 返回log_likelihood 而不是 likelihood

# 修改 initialize_parameters 函数
def initialize_parameters(K, p, X, y):
    """
...
def em_mshar(data_rv, max_iter=100, tol=1e-6):
    ...
    # ---> 修改初始化调用 <---
    betas, sigmas, P, pi = initialize_parameters(K, p, X, y)
    ...
    for iteration in range(max_iter):
        ...
        for k in range(K):
            ...
            if weighted_sum > 1e-10:
                ...
                if sigmas[k] < 1e-8:  # 防止sigma过小导致数值问题
                    sigmas[k] = 1e-8
            else:
                sigmas[k] = 0.01  # 或其他合理的默认值
        ...
        # 防止log_likelihood为nan或inf导致无限循环
        if not np.isfinite(log_likelihood):
            print(f"迭代 {iteration}: 对数似然值变为非有限数 ({log_likelihood})，停止迭代。")
            break
    ...
    model_info = {
        ...
        'converged': converged,
        'AIC': -2 * log_likelihood + 2 * (K * p + K + K * (K - 1)),
        'BIC': -2 * log_likelihood + np.log(T) * (K * p + K + K * (K - 1))
    }

    # 这里的参数数量是：K个状态 * p个系数 + K个sigma + K*(K-1)个独立的转移概率
    # 对于 K=2，就是 2*p + 2 + 2*(1) = 2p + 4. 你的代码中用了 2p + 2 + 2 = 2p+4，也是对的。
